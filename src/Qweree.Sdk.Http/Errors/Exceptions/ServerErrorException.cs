...
namespace Qweree.Sdk.Http.Errors.Exceptions
{
    ...
    {
        public ServerErrorException(int responseCode, ErrorResponseDto response) : this($"{responseCode} - {string.Join(", ", response.Errors?.Select(e => e.Message) ?? ImmutableArray<string?>.Empty)}", responseCode, response)
        {
        }
